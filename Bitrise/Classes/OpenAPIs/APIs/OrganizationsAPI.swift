...
open class OrganizationsAPI {
     ...
     */
    open class func orgList(apiResponseQueue: DispatchQueue = BitriseAPI.apiResponseQueue, completion: @escaping ((_ data: V0OrganizationListRespModel?,_ error: Error?) -> Void)) {
        orgListWithRequestBuilder().execute(apiResponseQueue) { result -> Void in
            switch result {
    ...
    open class func orgListWithRequestBuilder() -> RequestBuilder<V0OrganizationListRespModel> {
        ...
        let requestBuilder: RequestBuilder<V0OrganizationListRespModel>.Type = BitriseAPI.requestBuilderFactory.getBuilder()
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func orgShow(orgSlug: String, apiResponseQueue: DispatchQueue = BitriseAPI.apiResponseQueue, completion: @escaping ((_ data: V0OrganizationRespModel?,_ error: Error?) -> Void)) {
        orgShowWithRequestBuilder(orgSlug: orgSlug).execute(apiResponseQueue) { result -> Void in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    open class func orgShowWithRequestBuilder(orgSlug: String) -> RequestBuilder<V0OrganizationRespModel> {
        ...
        let requestBuilder: RequestBuilder<V0OrganizationRespModel>.Type = BitriseAPI.requestBuilderFactory.getBuilder()
